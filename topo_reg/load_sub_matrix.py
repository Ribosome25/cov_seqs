...
# For conveince in coding, just use pd df.to_dict(). Generic types would be better.
import numpy as np
import pandas as pd
...
#%%
all_matrices = {"benner6": benner6, "benner22": benner22,
                "benner74": benner74, "blosum100": blosum100,
                "blosum30": blosum30, "blosum35": blosum35,
                "blosum40": blosum40, "blosum45": blosum45,
                "blosum50": blosum50, "blosum55": blosum55,
                "blosum60": blosum60, "blosum62": blosum62,
                "blosum65": blosum65, "blosum70": blosum70,
                "blosum75": blosum75, "blosum80": blosum80,
                "blosum85": blosum85, "blosum90": blosum90,
                "blosum95": blosum95, "feng": feng,
                "fitch": fitch, "genetic": genetic,
                "gonnet": gonnet, "grant": grant,
                "ident": ident, "johnson": johnson,
                "levin": levin, "mclach": mclach,
                "miyata": miyata, "nwsgappep": nwsgappep,
                "pam120": pam120, "pam180": pam180,
                "pam250": pam250, "pam30": pam30,
                "pam300": pam300, "pam60": pam60,
                "pam90": pam90, "rao": rao,
                "risler": risler, "structure": structure
                }

def load_subs_mtrx(matrix='blosum50'):
    assert matrix in available_matrices, ("Unknown Substitution Matrix.")
    single_dic = all_matrices[matrix]

    aas = list("*ACBEDGFIHKMLNQPSRTWVYXZ")
    submtr_df = pd.DataFrame(2 * np.diag(np.ones(24)) - 1, index=aas, columns=aas)
    ending_score = [1]
    ending_score.extend([-5 for x in "ACBEDGFIHKMLNQPSRTWVYXZ"])
    submtr_df.loc['*'] = ending_score
    submtr_df.loc[:, '*'] = ending_score

    if matrix == "+-1":
        return submtr_df.to_dict()

    for each_key in single_dic:
        a = each_key[0]
        b = each_key[1]
        val = single_dic[each_key]
        submtr_df.loc[a, b] = val
        submtr_df.loc[b, a] = val
    return submtr_df.to_dict()
#%%
if __name__ == "__main__":
    from substitution_matrix import blosum50 as ref
    this = load_subs_mtrx()
