...
def load_starr2020():
    ...
    with open('../data/cov/starr2020cov2/expression_meanFs.csv') as f:
        f.readline()
        for line in f:
            ...
            # if len(mutants) > 7:  # 看一下到底有没有8+个突变的
            #     print("yes, 8+")
            mutable = [ aa for aa in wt_seq ]
            mut_pos = []
...
def self_load_gisaid():
    ...
    # import pandas as pd
    # t = pd.read_csv('/home/ruibzhan/Datasets/GISAID_0820/Seqs_w_meta_global.csv')
    # x = t['Seqs'].unique()
    # print(len(x))
    ...
    #         fields = line.replace('"', '').rstrip().split(',')
    #         log10Ka = np.nan
    #         mut_seq = fields[1].strip('*')  # check 原版应该没有 * 在结尾
    #         region = fields[4]
    #         date = fields[3]
    #         variant = fields[-1]
    #         gisaid_id = fields[0]
    # for 1203 added >>>>>
    if 1:
        df = pd.read_parquet('/home/ruibzhan/Datasets/GISAID_1203/GISAID1203+omicron.parquet')
        for idx, item in df.iterrows():
            log10Ka = np.nan
            mut_seq = item['Seqs'].strip('*')  # check 原版应该没有 * 在结尾
            region = item['region']
            date = item['date']
        ...
        # <<<<<<<<<
            if (mut_seq, strain) not in seqs_fitness:
                ...
                # raise ValueError("Please uncomment the follwoing lines.")
                seqs_fitness[(mut_seq, strain)][0][
                    'fitnesses'].append(log10Ka)
                seqs_fitness[(mut_seq, strain)][0][
                    'preferences'].append(log10Ka)
    ...
    print("GISAID total loaded:", len(seqs_fitness))
