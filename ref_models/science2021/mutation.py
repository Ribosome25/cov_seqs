...
def embed_seqs(args, model, seqs, vocabulary,
    ...
    """
    seqs 的结构：
    dict { sequence: list [metas]}
    meta 是dict，embedding 存在meta 的'embedding'里
    """
...
def analyze_comb_fitness(
    ...
    seqs_fitness = { seq: seqs_fitness[(seq, strain_i)]
                     ...
                     if strain_i == strain }
    # 注：seqs_fitness: dict {seq str : meta data}, size 105k
    ...
    for batchi in range(n_batches):
        ...
        # 这里返回的应该就是105k seqs 的 embedding (分batch)
        for mut_seq in seqs_fitness_batch:
            ...
            embd_last.append(meta['embedding'][-1])  # TODO
    ...
    df.to_parquet("../fitness_embeddings/CscsScore.parquet", compression='gzip')

    x_df = pd.DataFrame(embd_last)
    x_df.columns = ["F{}".format(x) for x in range(x_df.shape[1])]
    x_df.to_parquet("../fitness_embeddings/LSTM_Embeddings_Last.parquet", compression='gzip')
...
def embed_seqs_for_fitness(
        args, model, vocabulary, strain, wt_seq, seqs_fitness,
        comb_batch=None, prob_cutoff=0., beta=1., verbose=True,
        part=None # expression embed 老是OOM。 用这个分成两部分
        ):
    # 注 seqs_fitness 是dataset，格式是{seq string ： list [meta dic, ...]}
    from copy import deepcopy
    seqs_fitness = { seq: seqs_fitness[(seq, strain_i)]
                     for seq, strain_i in seqs_fitness
                     if strain_i == strain }
    ...
    for batchi in range(n_batches):
        ...
        # 这里返回的应该就是105k seqs 的 embedding (分batch)
        # 格式是{mut seq: list[meta {'embedding': embedding, 'strain': .., ..}]} (check dim of embd)
        for mut_seq in seqs_fitness_batch:
            # 注 iterate 处理batch 的每条内容
            assert(len(seqs_fitness_batch[mut_seq]) == 1)
            meta = seqs_fitness_batch[mut_seq][0]
            if meta['strain'] != strain:
                continue
            ...
            data.append([
                str(mut_seq), # check type(mut_seq)
                meta['strain'],
                meta['fitness'],
                meta['preference'],
                sem_change
            ...
            embd.append(meta['embedding'])
            embd_mean.append(meta['embedding'].mean(0))
            embd_last.append(meta['embedding'][-1])

        # np.save("../fitness_embeddings/Batch_{}.npy".format(batchi), np.array(embd))
        del seqs_fitness_batch

    y_df = pd.DataFrame(data, columns=[
        "Seqs",
        'strain', 'fitness', 'preference', 'sem_change'])

    #%% Save outputs
    # y_df.to_parquet("../fitness_embeddings/Seqs_Expression_part{}.parquet".format(part), compression='gzip')
    # y_df.to_parquet("../GisAid_0820/Seqs_0820_allmeta.parquet", compression='gzip')
    y_df.to_parquet("../GisAid_1203/Seqs_1203_sem_change.parquet", compression='gzip')
    del data, y_df

    # with open("../fitness_embeddings/Embeddings_Mean_expression.pickle", 'wb') as f:
        # pickle.dump(embd_mean, f)
    # with open("../fitness_embeddings/Embeddings_Last_expression.pickle", 'wb') as f:
        # pickle.dump(embd_last, f)

    x_df = pd.DataFrame(embd_mean)
    x_df.columns = ["F{}".format(x) for x in range(x_df.shape[1])]
    # x_df.to_parquet("../fitness_embeddings/Embeddings_Mean_expression_part{}.parquet".format(part), compression='gzip')
    # x_df.to_parquet("../GisAid_0820/Embeddings_Mean.parquet", compression='gzip')
    x_df.to_parquet("../GisAid_1203/Embeddings_Mean_part{}.parquet".format(part), compression='gzip')
    del embd_mean, x_df
    x2_df = pd.DataFrame(embd_last)
    x2_df.columns = ["F{}".format(x) for x in range(x2_df.shape[1])]
    # x2_df.to_parquet("../fitness_embeddings/Embeddings_Last_expression_part{}.parquet".format(part), compression='gzip')
    # x2_df.to_parquet("../GisAid_0820/Embeddings_Last.parquet", compression='gzip')
    x_df.to_parquet("../GisAid_1203/Embeddings_Last_part{}.parquet".format(part), compression='gzip')
    return None
